            ...
            for i in scraped_data:
                ...
                for ctxt in scraped_data[i]:
                    if type(ctxt).__name__=="dict":
                        if type(ctxt.get('url')) == str: # making url list
                            ctxt['url'] = [ctxt['url']]
                        if ctxt.get('url'): # renaming url to urls.
                            ctxt['urls'] = ctxt['url']
                            del ctxt['url']
                        ...
                        count =0 
                        for dct in ctxt:
                            if type(dct.get('url')) == str: # making url list
                                dct['urls'] = [dct['url']]
                            if dct.get('url'): # renaming url to urls.
                                dct['urls'] = dct['url']
                                del dct['url']
                            if InputParameters['AppName']==   "Revenue Cycle Management":
                                ctxt["PMSSubscriberId"]=patient.get("SubscriberId")                                                      
...
d=json.loads(open("deltadentalohio.json","r").read())
