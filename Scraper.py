...
import os
import io
from common.functions import screenshot,upload_image_to_azure,screenshot_pagewise,ElementSCreenhot,upload_pdf_to_azure
...
class Scraper:
    ...
    def merge_pdf_files(self):
        ...
        if patient_id:
            ...
            if os.path.exists(os.path.join(patient_pdf_dir, "merged_pdf.pdf")):
                os.remove(os.path.join(patient_pdf_dir, "merged_pdf.pdf"))
            ...
            if os.path.exists(patient_screenshot_pdf_path) or os.listdir(patient_pdf_dir):
                merged_pdf_path = os.path.join(patient_pdf_dir, "merged_pdf.pdf")
                merger.write(merged_pdf_path)
            merger.close()
        return merged_pdf_path

    def remove_processed_pdf_dir(self):
        pass
    ...
    def clicker(self,wait,xpath):
        ...
        try:
            ...
            try:
                ...
                try:
                    t=self.browser.execute_script("try{document.evaluate(arguments[0],document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.scrollIntoView();return true;}catch(err){return false;}", xpath)
                    t=self.browser.execute_script("try{document.evaluate(arguments[0],document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue.click();return true;}catch(err){return false;}", xpath)  
                    if t==True:
    ...
    def Scrap(self):
            ...
            if multiple_elements:
                ...
                if multiple_elements_xpath:
                    ...
                    if multiple_elements.get("multiple_elements_type")==None:
                        multiple_elements_xpath=  [x.get_attribute("textContent") for x in self.browser.find_elements(By.XPATH,multiple_elements_xpath)]
                    ...
                    if multiple_elements.get("multiple_elements_type")  =="HrefBased":
                        multiple_elements_xpath= [x.get_attribute("href") for x in self.browser.find_elements(By.XPATH,multiple_elements_xpath)]
            ...
            for element_ in  multiple_elements_xpath:
                ...
                if action =="Click":  
                    ...
                    if multiple_elements.get("multiple_elements_type")=="XpathBased":      
                        ...
                        wait.until(EC.element_to_be_clickable((By.XPATH,f"//*[contains(text(),'{element_}')]"))).click()
                ...
                def extract(j):
                    ...
                    if multiple_elements_:                                
                        ...
                        try:
                            if multiple_elements_.get("type"):
                                ...
                                multiple_elements_xpath_=  [x.get_attribute("id") for x in self.browser.find_elements(By.XPATH,multiple_elements_xpath_)]
                        except:
                            multiple_elements_xpath_= range(1)   
                    ...
                    for elem in  multiple_elements_xpath_:
                        ...
                        if j["AdditonalInfo"].get("DeleteElement"):
                            self.browser.execute_script("var element = document.evaluate(arguments[0], document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;element.parentNode.removeChild(element);",j["AdditonalInfo"].get("DeleteElement"))                                       
                            sleep(3)      
                            ...
                            if scrap ==True:
                                ...
                                try: 
                                    ...
                                    if j['AdditonalInfo'].get("waitTime",7)==0:
                                        ...
                                        try:                                            
                                            WebDriverWait(self.browser,j['AdditonalInfo'].get("waitTime",7)).until(EC.presence_of_element_located((By.XPATH,xpath))) 
                                        except:
                                            pass    
                                           
                                    if self.Element_with_retry(xpath,WebDriverWait(self.browser,j['AdditonalInfo'].get("waitTime",7)),tag_name=True) =='table':
                                      
                                        
                                        html=self.Element_with_retry(xpath,WebDriverWait(self.browser,j['AdditonalInfo'].get("waitTime",7)),html=True)
                                        ...
                                        #if dictionary 
                                            if len(key)!=0:
                                                df=pd.DataFrame([data_]).set_axis(key, axis=1, inplace=False).to_dict('records') 
                                            else:
                                                df=data_            
                                        ...
                                        if len(key)!=0: #if list
                                            df_=pd.DataFrame(data_).set_axis(key, axis=1, inplace=False).to_dict('records')  
                                        ...
                                        if type(key).__name__=='list':
                                            ...
                                            for i in rows:
                                                for z in i.findAll(recursive=False):
                                                    final_rows.append([neat(x.text) for x in z.findAll(recursive=False)])  
                                                    
                                            if len(final_rows[0]) ==2: 
                                                df =pd.DataFrame(final_rows)
                                                
                                                if len(key) !=0:                   
                                                    df=pd.DataFrame([dict(zip(df[0], df[1]))]).set_axis(key, axis=1, inplace=False).to_dict('records')
                                                else:
                                                ...
                                                if len(key)!=0:
                                                    df=pd.DataFrame(final_rows).set_axis(key, axis=1, inplace=False).to_dict('records')
                                                for dc in df:
                                                    data[DataContextName].append(self.keyupdater(dc,mandatoryfields))                                 
                                            
                                        else:                                        
                                            value=self.Element_with_retry(xpath,WebDriverWait(self.browser,j['AdditonalInfo'].get("waitTime",7)),text=True)
                                            d[key]=value
                                            print(d)                                                                                                                                    
        ...
        if merged_pdf_path:
            pdf_blob_url =upload_pdf_to_azure(merged_pdf_path,"files",os.environ['ConnectionString'])
            print("Merged Pdf File Blob Details: ", pdf_blob_url)
            data["MergedPDFFile"]=[]
            data["MergedPDFFile"].append({"PatientId":self.patientData.get("PatientId"),"BlobUrl":pdf_blob_url})
        self.remove_directories()
        return data
